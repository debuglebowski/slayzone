...
function reconcileLinearStateMappingsForProject(
  ...
  for (const mapping of mappings) {
    ...
    for (const row of nextRows) {
      insertStateMapping.run(randomUUID(), mapping.id, row.localStatus, row.stateId, row.stateType)
    }
  }
}
...
export function listAllProjects(db: Database): Project[] {
  ...
  return rows.map((row) => parseProject(row)).filter((p): p is Project => p !== null)
}

export function createProject(db: Database, data: CreateProjectInput): Project {
  ...
  return db.transaction(() => {
    ...
    return parseProject(row) as unknown as Project
  })()
}

export function updateProject(db: Database, data: UpdateProjectInput): Project {
  const fields: string[] = []
  ...
  if (data.autoCreateWorktreeOnTaskCreate !== undefined) {
    ...
    if (data.autoCreateWorktreeOnTaskCreate === null) values.push(null)
    else values.push(data.autoCreateWorktreeOnTaskCreate ? 1 : 0)
  }
  if (data.worktreeSourceBranch !== undefined) { fields.push('worktree_source_branch = ?'); values.push(data.worktreeSourceBranch) }
  if (data.worktreeCopyBehavior !== undefined) { fields.push('worktree_copy_behavior = ?'); values.push(data.worktreeCopyBehavior) }
  if (data.worktreeCopyPaths !== undefined) { fields.push('worktree_copy_paths = ?'); values.push(data.worktreeCopyPaths) }
  if (data.worktreeSubmoduleInit !== undefined) { fields.push('worktree_submodule_init = ?'); values.push(data.worktreeSubmoduleInit) }
  ...
  if (data.selectedRepo !== undefined) { fields.push('selected_repo = ?'); values.push(data.selectedRepo) }
  ...
  if (data.iconImagePath !== undefined) {
    ...
    if (data.iconImagePath === null) unlinkProjectIconFiles(data.id)
...
export function uploadProjectIcon(db: Database, projectId: string, sourcePath: string): Project {
...
export function reorderProjects(db: Database, projectIds: string[]): void {
