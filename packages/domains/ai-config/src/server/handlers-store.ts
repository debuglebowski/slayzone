...
async function queryProjectSkillSelections(
  db: SlayzoneDb,
  projectId: string
): Promise<ProjectSkillSelectionRow[]> {
  return (await db
    .prepare(`
...
export function createAiConfigOps(db: SlayzoneDb) {
  ...
  async function getDefaultProviderKind(): Promise<CliProvider> {
    const mode = (
      (await db.prepare("SELECT value FROM settings WHERE key = 'default_terminal_mode'").get()) as
        | { value: string }
        | undefined
    )?.value
  ...
  async function getEnabledProviders(projectId: string): Promise<CliProvider[]> {
    ...
    const defaultKind = await getDefaultProviderKind()
  ...
  async function recomputeInstructionsResult(
    projectId: string,
    projectPath: string
    ...
    const providers = await getEnabledProviders(projectId)
    const resolvedProject = path.resolve(projectPath)
  ...
  async function recomputeSingleSkillStatus(
    projectId: string,
    projectPath: string,
    itemId: string
    ...
    const rawItem = (await db.prepare('SELECT * FROM ai_config_items WHERE id = ?').get(itemId)) as
      | AiConfigItem
      | undefined
  ...
  const computeStaleSkillCount = async (
    projectId: string,
    projectPath: string
  ): Promise<number> => {
    const providers = await getEnabledProviders(projectId)
    ...
    const selections = await queryProjectSkillSelections(db, projectId)
  ...
  return {
    listItems: async (input: ListAiConfigItemsInput) => {
    ...
    getItem: async (id: string) => {
      const row = (await db.prepare('SELECT * FROM ai_config_items WHERE id = ?').get(id)) as
        | AiConfigItem
        | undefined
      return row ? withDerivedSkillValidation(row) : null
    },
    createItem: async (input: CreateAiConfigItemInput) => {
    ...
    updateItem: async (input: UpdateAiConfigItemInput) => {
      ...
      if (fields.length > 0) {
        ...
        try {
          await db
            .prepare(`UPDATE ai_config_items SET ${fields.join(', ')} WHERE id = ?`)
            .run(...values)
        } catch (error) {
          if (isUniqueConstraintError(error)) {
      ...
      const row = (await db.prepare('SELECT * FROM ai_config_items WHERE id = ?').get(input.id)) as
        | AiConfigItem
        | undefined
      return row ?? null
    },
    deleteItem: async (id: string) => {
      await db.prepare('DELETE FROM ai_config_project_selections WHERE item_id = ?').run(id)
      const result = await db.prepare('DELETE FROM ai_config_items WHERE id = ?').run(id)
      return result.changes > 0
    },
    listProjectSelections: async (projectId: string) => {
      const rows = (await db
        .prepare(`
    ...
    setProjectSelection: async (input: SetAiConfigProjectSelectionInput) => {
    ...
    removeProjectSelection: async (projectId: string, itemId: string, provider?: string) => {
      if (provider) {
    ...
    discoverContextFiles: (projectPath: string) => {
      ...
      const computerClaude = path.join(homedir(), '.claude', 'CLAUDE.md')
      results.push({
        path: computerClaude,
      ...
      const computerCopilot = path.join(homedir(), '.copilot', 'AGENTS.md')
      results.push({
    ...
    writeComputerSkill: (provider: CliProvider, slug: string, content: string) => {
      if (!isConfigurableCliProvider(provider))
        throw new Error(`Provider ${provider} is not configurable`)
      const spec = COMPUTER_PROVIDER_PATHS[provider]
    ...
    getContextTree: async (projectPath: string, projectId: string) => {
    ...
    loadLibraryItem: async (input: LoadLibraryItemInput) => {
      const item = (await db
        .prepare('SELECT * FROM ai_config_items WHERE id = ?')
        .get(input.itemId)) as AiConfigItem | undefined
      if (!item) throw new Error('Item not found')
      if (item.type === 'skill') {
    ...
    syncLinkedFile: async (
      ...
      `)
        ...
        const providersToSync = provider
          ...
          : await getEnabledProviders(projectId)
    ...
    `)
      ...
      for (const sel of selections) {
        ...
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
        fs.writeFileSync(filePath, syncedContent, 'utf-8')
        await updateSelection.run(effectiveTargetPath, contentHash(syncedContent), sel.id)
        removeLegacySkillFileIfPresent(
          providerId,
          sel.item_type,
          sel.item_slug,
    ...
    unlinkFile: async (projectId: string, itemId: string) => {
      const result = await db
        .prepare(`
    ...
    renameContextFile: async (oldPath: string, newPath: string, projectPath: string) => {
    ...
    deleteContextFile: async (filePath: string, projectPath: string, projectId: string) => {
      if (!isPathAllowed(filePath, projectPath)) throw new Error('Path not allowed')
    ...
    getLibraryInstructions: async (variantId?: string) => {
      if (variantId) {
        const item = (await db
          .prepare(
            "SELECT * FROM ai_config_items WHERE id = ? AND type = 'root_instructions' AND scope = 'library'"
    ...
    saveLibraryInstructions: async (content: string, variantId?: string) => {
      if (variantId) {
        await db
          .prepare(
    ...
    listInstructionVariants: async (): Promise<AiConfigItem[]> => {
      return (await db
        .prepare(
    ...
    getProjectInstructionVariant: async (projectId: string): Promise<AiConfigItem | null> => {
      const row = (await db
        .prepare('SELECT value FROM settings WHERE key = ?')
        .get(`ai_instruction_variant:${projectId}`)) as { value: string } | undefined
      if (!row) return null
    ...
    setProjectInstructionVariant: async (
      projectId: string,
      ...
      if (variantItemId === null) {
        await db
          ...
          .run(`ai_instruction_variant:${projectId}`)
        return
      }
    ...
    saveInstructionsContent: async (
      ...
      return await recomputeInstructionsResult(projectId, projectPath)
    },
    saveRootInstructions: async (projectId: string, projectPath: string, content: string) => {
    ...
    readProviderInstructions: (projectPath: string, provider: CliProvider): ProviderFileContent => {
      const rootPath = PROVIDER_PATHS[provider]?.rootInstructions
    ...
    pushProviderInstructions: async (
      projectId: string,
      projectPath: string,
      provider: CliProvider,
      content: string
    ...
    pullProviderInstructions: async (
      projectId: string,
      projectPath: string,
      provider: CliProvider
    ): Promise<RootInstructionsResult> => {
    ...
    getProjectSkillsStatus: async (projectId: string, projectPath: string) => {
      const providers = await getEnabledProviders(projectId)
      const resolvedProject = path.resolve(projectPath)
    ...
    readProviderSkill: async (
      projectPath: string,
      provider: CliProvider,
      itemId: string
    ): Promise<ProviderFileContent> => {
      const item = (await db.prepare('SELECT * FROM ai_config_items WHERE id = ?').get(itemId)) as
        | AiConfigItem
        | undefined
    ...
    pullProviderSkill: async (
      ...
      const pulledSelection = (await db
        .prepare(
      ...
      return await recomputeSingleSkillStatus(projectId, projectPath, itemId)
    },
    listProviders: async () => {
    ...
    toggleProvider: async (id: string, enabled: boolean) => {
      const row = (await db.prepare('SELECT kind FROM ai_config_sources WHERE id = ?').get(id)) as
        | { kind: string }
        | undefined
      if (!row || !isConfigurableCliProvider(row.kind)) return
      // Prevent disabling the default terminal mode's provider
      if (!enabled && row.kind === (await getDefaultProviderKind())) return
      await db
        .prepare(
    ...
    reconcileProjectSkills: async (projectId: string, projectPath: string) => {
      const resolvedProject = path.resolve(projectPath)
      const enabledProviders = new Set(await getEnabledProviders(projectId))
      let created = 0
    ...
    getProjectProviders: async (projectId: string) => {
      const row = (await db
        .prepare('SELECT value FROM settings WHERE key = ?')
        .get(`ai_providers:${projectId}`)) as { value: string } | undefined
      if (row) {
    ...
    setProjectProviders: async (projectId: string, providers: CliProvider[]) => {
      await db
        .prepare(
          'INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value'
        )
    ...
    needsSync: async (projectId: string, projectPath: string): Promise<boolean> => {
      const providers = await getEnabledProviders(projectId)
    ...
    getProjectsStaleSkillCounts: async (
      pairs: Array<{ projectId: string; projectPath: string }>
    ): Promise<Record<string, number>> => {
      const out: Record<string, number> = {}
      for (const { projectId, projectPath } of pairs) {
        out[projectId] = await computeStaleSkillCount(projectId, projectPath)
      }
      return out
    },
    syncAll: async (input: SyncAllInput) => {
      const resolvedProject = path.resolve(input.projectPath)
    ...
    `)
      ...
      for (const sel of selections) {
        ...
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
        fs.writeFileSync(filePath, syncedContent, 'utf-8')
        await updateSelection.run(effectiveTargetPath, hash, sel.id)
        removeLegacySkillFileIfPresent(
          provider,
          sel.item_type,
          sel.item_slug,
    ...
    checkSyncStatus: async (projectId: string, projectPath: string) => {
      const resolvedProject = path.resolve(projectPath)
    ...
    discoverMcpConfigs: (projectPath: string): McpConfigFileResult[] => {
      return discoverMcpConfigsFromSpecs(path.resolve(projectPath), PROJECT_MCP_SPECS)
    },
    writeMcpServer: (input: WriteMcpServerInput) => {
    ...
    discoverComputerMcpConfigs: (): McpConfigFileResult[] => {
      return discoverMcpConfigsFromSpecs(homeDir, COMPUTER_MCP_SPECS)
    },
    writeComputerMcpServer: (input: WriteComputerMcpServerInput) => {
      writeMcpServerToSpecs(
        homeDir,
        COMPUTER_MCP_SPECS,
        input.provider,
    ...
    removeComputerMcpServer: (input: RemoveComputerMcpServerInput) => {
      removeMcpServerFromSpecs(homeDir, COMPUTER_MCP_SPECS, input.provider, input.serverKey)
    },
    checkSlayConfigured: (projectPath: string): boolean => {
    ...
    setupSlay: (projectPath: string, projectId?: string): { ok: boolean; error?: string } => {
      try {
        const cwd = path.resolve(projectPath)
        const devFlag = process.env.NODE_ENV === 'development' ? ' --dev' : ''
