...
export function createMarketplaceOps(db: SlayzoneDb) {
  ...
  return {
    ...
    addRegistry: async (input: AddRegistryInput) => {
    ...
    toggleRegistry: async (registryId: string, enabled: boolean) => {
      await db
        .prepare(
    ...
    refreshRegistry: async (registryId: string) => {
      try {
    ...
    listEntries: async (input?: ListEntriesInput) => {
    ...
    unlinkSkill: async (itemId: string) => {
      const item = (await db
