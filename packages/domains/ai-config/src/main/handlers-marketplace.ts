...
export function registerMarketplaceHandlers(ipcMain: IpcMain, db: SlayzoneDb): void {
  // Ensure built-in entries are seeded. Fire-and-forget (was synchronous at
  // registration before the async DB lift) — handlers are async and the seed
  // settles well before any user interaction.
  void db
    .namedTxn('ai-config:marketplace:seed-builtin-entries', {})
    .catch((err) => console.error('[marketplace] Failed to seed builtin entries:', err))

  ipcMain.handle('ai-config:marketplace:list-registries', async () => {
    const rows = (await db
      .prepare(`
  ...
  ipcMain.handle('ai-config:marketplace:remove-registry', async (_event, registryId: string) => {
    ...
    await db.prepare('DELETE FROM skill_registries WHERE id = ?').run(registryId)
    return true
  })

  ipcMain.handle(
    'ai-config:marketplace:toggle-registry',
    async (_event, registryId: string, enabled: boolean) => {
      await db
        .prepare(
  ...
  ipcMain.handle('ai-config:marketplace:refresh-all', async () => {
    const registries = (await db
      .prepare('SELECT id FROM skill_registries WHERE enabled = 1')
      .all()) as {
      id: string
    }[]
  ...
  ipcMain.handle('ai-config:marketplace:list-entries', async (_event, input?: ListEntriesInput) => {
  ...
  ipcMain.handle('ai-config:marketplace:install-skill', async (_event, input: InstallSkillInput) => {
    ...
    if (existing)
      return await db.prepare('SELECT * FROM ai_config_items WHERE id = ?').get(existing.id)
    ...
    if (slugConflict)
      return await db.prepare('SELECT * FROM ai_config_items WHERE id = ?').get(slugConflict.id)
    ...
    const registry = (await db
      .prepare('SELECT name FROM skill_registries WHERE id = ?')
      .get(registryId)) as { name: string } | undefined
    ...
    return await db.prepare('SELECT * FROM ai_config_items WHERE id = ?').get(id)
  })

  ipcMain.handle('ai-config:marketplace:check-updates', async () => {
    const rows = (await db
      .prepare(`
  ...
  ipcMain.handle('ai-config:marketplace:unlink-skill', async (_event, itemId: string) => {
    const item = (await db
    ...
    return await db.prepare('SELECT * FROM ai_config_items WHERE id = ?').get(itemId)
  })

  ipcMain.handle(
    'ai-config:marketplace:update-skill',
    async (_event, itemId: string, entryId: string) => {
      const entry = (await db
        .prepare('SELECT * FROM skill_registry_entries WHERE id = ?')
...
async function refreshRegistry(db: SlayzoneDb, registryId: string): Promise<SkillRegistryEntry[]> {
  const registry = (await db
    .prepare('SELECT * FROM skill_registries WHERE id = ?')
    .get(registryId)) as Record<string, unknown> | undefined
  if (!registry) throw new Error('Registry not found')

  const sourceType = registry.source_type as string

  if (sourceType === 'builtin') {
    await db.namedTxn('ai-config:marketplace:seed-builtin-entries', {})
    const rows = (await db
  ...
  if (sourceType === 'github') {
    ...
    // Upsert entries in a transaction. Conditional read-modify-write (reads
    // current slugs to prune missing ones), so it runs as a named txn inside
    // the DB worker — see marketplace-txns.ts.
    await db.namedTxn('ai-config:marketplace:refresh-registry-entries', {
      registryId,
