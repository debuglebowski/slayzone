/**
 * Hub-side runner gateway. Owns every runner WebSocket that dialed in, speaks
 * the duplex JSON-RPC runner protocol over each, and exposes an addressable
 ...
 *     `runner-disconnected`.
 *
 * @module runner/server/hub-gateway
 */
...
export type RunnerGatewayEvents = {
...
export interface HubRunnerGatewayOptions {
  /**
...
export interface HubRunnerGateway {
...
export function createHubRunnerGateway(options: HubRunnerGatewayOptions): HubRunnerGateway {
  const heartbeatTimeoutMs = options.heartbeatTimeoutMs ?? DEFAULT_HEARTBEAT_TIMEOUT_MS
  const authTimeoutMs = options.authTimeoutMs ?? DEFAULT_AUTH_TIMEOUT_MS
  const requestTimeoutMs = options.requestTimeoutMs ?? DEFAULT_REQUEST_TIMEOUT_MS
  const log = options.log ?? (() => {})

  const events = new TypedEventEmitter<RunnerGatewayEvents>((event, err) => {
    log('runner gateway listener threw', { event: String(event), error: String(err) })
  })
  ...
  async function handleEnroll(conn: RunnerConnection, params: unknown, id: JsonRpcId): Promise<void> {
    ...
    if (!parsed.success) {
      conn.rpc.respondError(id, RunnerTransportErrorCodes.unauthorized, 'malformed enroll request')
      protocolError(conn, `malformed enroll params: ${parsed.error.message}`)
      conn.ws.close(1008, 'malformed enroll')
      return
    }
    if (parsed.data.protocolVersion !== RUNNER_PROTOCOL_VERSION) {
      conn.rpc.respondError(
        id,
        RunnerTransportErrorCodes.protocolMismatch,
        `hub speaks runner protocol v${RUNNER_PROTOCOL_VERSION}, runner sent v${parsed.data.protocolVersion}`
      )
    ...
    try {
      ...
      const code = err instanceof RpcError ? err.code : RunnerTransportErrorCodes.unauthorized
    ...
    log('runner runner enrolled', { runnerId: minted.runnerId, name: descriptor.name })
  }
  ...
  async function handleHello(conn: RunnerConnection, params: unknown, id: JsonRpcId): Promise<void> {
    ...
    if (!parsed.success) {
      conn.rpc.respondError(id, RunnerTransportErrorCodes.unauthorized, 'malformed hello request')
      conn.ws.close(1008, 'malformed hello')
      return
    }
    let identity: Awaited<ReturnType<HubRunnerGatewayOptions['verifyApiKey']>>
    try {
    ...
    if (!identity) {
      ...
      conn.rpc.respondError(id, RunnerTransportErrorCodes.unauthorized, 'unknown api key')
      return
    }
    const descriptor: RunnerDescriptor = {
    ...
    events.emit('runner-connected', { runner: { ...descriptor } })
    log('runner runner reconnected', { runnerId: identity.runnerId })
  }
  ...
  function handleRunnerRequest(conn: RunnerConnection, method: string, params: unknown, id: JsonRpcId): void {
    if (!conn.descriptor) {
      if (method === RunnerToHubMethods.enroll) {
        ...
        conn.rpc.respondError(id, RunnerTransportErrorCodes.unauthorized, 'authenticate with enroll or hello first')
    ...
    switch (method) {
      ...
      case RunnerToHubMethods.hello:
        conn.rpc.respondError(id, RunnerTransportErrorCodes.unauthorized, 'session already authenticated')
        return
      default:
  ...
  function handleConnection(ws: WebSocket): void {
    ...
    ws.on('error', (err) => {
      log('runner runner socket error', { runnerId: conn.descriptor?.runnerId, error: String(err) })
    })
  }
    ...
    request<T = unknown>(runnerId: string, method: string, params?: unknown, timeoutMs?: number): Promise<T> {
      ...
      if (!conn) {
        return Promise.reject(
          new RpcError(RunnerTransportErrorCodes.unknownRunner, `runner '${runnerId}' is not connected`)
        )
      }
