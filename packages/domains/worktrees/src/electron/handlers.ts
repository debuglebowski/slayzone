...
import { runAiCommand } from '../server/merge-ai'
import { listProjectRepos } from '../server/list-project-repos'
import { ensureColors } from '../server/color-registry'
import {
  checkGhInstalled,
  hasGithubRemote,
  listOpenPrs,
...
import { resolveCopyBehavior, resolveSubmoduleInitBehavior } from '../server/copy-behavior'
export { resolveCopyBehavior, resolveSubmoduleInitBehavior }
...
export function registerWorktreeHandlers(ipcMain: IpcMain, db: Database): void {
  // Watcher → renderer broadcast bridge wired by Electron-main glue
  // (packages/apps/app/src/main/glue/index.ts). Server pkg uses tRPC
