...
/**
 * Fleet settings (hub/runner split, wave 3). The fleet toggle is backed by the
 * pre-boot config file — NOT the settings DB — because the hub gateway + auth +
 * runners deps are decided at boot (`SLAYZONE_FLEET_MODE`, see
 * boot-config.ts:fleetEnvFor). So the mode is read via `getBootConfig` and
 ...
 * `runners.list` degrades gracefully with fleet off (store rows, all
 * disconnected); `runners.mintJoinToken` REQUIRES the bound hub listener and
 * throws when fleet is off — so enrollment is gated on the *booted* fleet state
 * (`savedFleet`) and mint errors are surfaced as a toast rather than crashing.
 */
...
export function FleetSettingsTab() {
  ...
  // The store-backed list works regardless of fleet mode (pure DB read; the live
  ...
  useEffect(() => {
    void electronBootstrap.getBootConfig().then((cfg) => {
      setFleet(cfg.fleetMode)
      setSavedFleet(cfg.fleetMode)
      setLoaded(true)
    })
  }, [])
  ...
  const dirty = fleet !== savedFleet

  const save = async (): Promise<void> => {
    setSaving(true)
    try {
      await electronBootstrap.setBootSettings({ fleet_mode: fleet })
      // Fleet mode is decided at boot (the sidecar's hub gateway/auth/runners
      // deps are gated on SLAYZONE_FLEET_MODE), so a change needs a relaunch. The
  ...
  // Toggling fleet off should not leave a live one-time enrollment secret on
  // screen — a token minted while fleet was on is meaningless once the user
  // decides to turn fleet off, so drop it.
  const onFleetChange = (next: boolean): void => {
    setFleet(next)
  ...
  return (
    <div className="space-y-6">
      <SettingsTabIntro
        title="Fleet"
        description="Run terminals and agents on remote runner machines. With fleet mode on, this app becomes a hub that enrolled runners dial into over an authenticated connection — task work can then execute on those machines instead of locally."
      />

      <div className="space-y-3">
        <Label className="text-base font-semibold">Fleet mode</Label>
        <div className="flex items-start gap-3">
          <Switch
            checked={fleet}
            disabled={!loaded}
            onCheckedChange={onFleetChange}
            data-testid="fleet-mode-toggle"
          />
          <div>
            <div className="text-sm font-medium">Enable fleet mode</div>
        ...
        <div className="flex items-center gap-2 pt-2">
          <Button
            ...
            data-testid="fleet-save-relaunch"
          >
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center gap-2">
            <Input
              ...
              disabled={!savedFleet || minting}
              className="max-w-xs"
              data-testid="fleet-runner-label"
            />
            <Button
              variant="outline"
              disabled={!savedFleet || minting}
          ...
          {!savedFleet && (
            <p className="text-muted-foreground text-xs" data-testid="fleet-enroll-disabled">
              Turn fleet mode on and relaunch to enroll runners — the hub listener has to be bound
          ...
          {mintedToken && (
            <div
              ...
              data-testid="fleet-minted-token"
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Button
                    ...
                    data-testid="fleet-token-dismiss"
                  >
      ...
      <Card>
        ...
        {runners.length > 0 && (
          <CardContent>
            <table className="w-full text-sm" data-testid="fleet-runners-table">
              <thead>
