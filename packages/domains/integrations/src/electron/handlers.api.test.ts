...
try {
  ...
  no('re-connect updates existing', e)
}
...
try {
  ...
  no('returns projects for team', e)
}
...
try {
  ...
  no('creates mapping and refreshes state mappings', e)
}
...
try {
  ...
  no('upserts on same project+provider', e)
}
...
try {
  ...
  no('annotates linkedTaskId for linked issues', e)
}
...
try {
  ...
  no('returns null linkedTaskId for unlinked issues', e)
}
...
try {
  ...
  no('imports all issues with correct priority + state mapping', e)
}
...
try {
  ...
  no('selectedIssueIds filters imports', e)
}
...
try {
  ...
  no('re-import updates existing (dedup)', e)
}
...
try {
  ...
  no('markdown description converted to HTML', e)
}
...
try {
  ...
  no('throws when no team + no mapping', e)
}
...
try {
  ...
  no('pulls remote when remote is newer', e)
}
...
try {
  ...
  no('does NOT push when one_way', e)
}
...
try {
  ...
  no('records error when API throws', e)
}
...
function setupGitProject(db: import('better-sqlite3').Database, projectId: string): string {
  ...
  db.prepare(`INSERT OR REPLACE INTO settings (key, value) VALUES ('worktree_base_path', ?)`).run(
...
try {
  ...
  no('Linear import provisions worktree when setting=1 (issue #84)', e)
}
...
try {
  ...
  no('Linear import skips worktree when setting=0', e)
}
...
try {
  ...
  no('Re-import updates existing task without disturbing worktree', e)
}
