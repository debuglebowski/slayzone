...
// 1. Local hook-capable agent → loopback hook URL, no port env, no hub env.
{
  ...
  assert(!('SLAYZONE_SERVER_PORT' in env), 'local must NOT set SLAYZONE_SERVER_PORT (CLI reads DB)')
...
// 3. Local non-hook-capable mode → no hook URL, no port env (unchanged behavior).
{
  ...
  assert(!('SLAYZONE_SERVER_PORT' in env), 'no port env for non-hook mode either')
...
// 4. Remote hook-capable → hub URL + hub hook URL + bearer, NO loopback port.
{
  ...
  assert(!('SLAYZONE_SERVER_PORT' in env), 'remote must NOT set loopback SLAYZONE_SERVER_PORT')
...
// 6. Remote non-hook-capable → hub URL/token but no hook URL.
{
  ...
  assert(!('SLAYZONE_SERVER_PORT' in env), 'still no loopback port remotely')
}
