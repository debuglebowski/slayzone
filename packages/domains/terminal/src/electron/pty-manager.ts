...
import { getDiagnosticsConfig, recordDiagnosticEvent } from '@slayzone/diagnostics/electron'
...
/** Electron-side handler for "task reached a terminal status". Wired to the
 *  shared `setOnTaskReachedTerminalHandler` (server/task-events.ts) at boot
 *  so other domains can call `onTaskReachedTerminal(taskId)` server-safely. */
export function electronOnTaskReachedTerminal(taskId: string): void {
