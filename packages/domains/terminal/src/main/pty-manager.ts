...
interface PtySession {
  ...
  // Codex wrapper TUI session log path. Pre-generated by pty-manager and
  // injected as $CODEX_TUI_SESSION_LOG_PATH so the wrapper bash script tails
  // it. Removed on PTY exit. Undefined for non-codex modes.
...
export async function createPty(opts: CreatePtyOptions): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    // Codex wrapper TUI session log + PATH prepend so the codex binary is
    // shadowed by ~/.slayzone/bin/codex for this PTY only. Skipped for
    // transport (docker/ssh) PTYs — wrapper requires local FS access.
    let codexSessionLogPath: string | undefined
    const codexEnv: Record<string, string> = {}
    if (terminalMode === 'codex') {
      codexSessionLogPath = path.join(tmpdir(), `slayzone-codex-session-${randomUUID()}.jsonl`)
      codexEnv.CODEX_TUI_RECORD_SESSION = '1'
      codexEnv.CODEX_TUI_SESSION_LOG_PATH = codexSessionLogPath
      const slayzoneBin = path.join(getSlayzoneHomeDir(), 'bin')
      const existingPath = process.env.PATH ?? ''
      codexEnv.PATH = existingPath ? `${slayzoneBin}:${existingPath}` : slayzoneBin
    }
    ...
    const finalizeSessionExit = (exitCode: number): void => {
      ...
      if (exitSession?.codexSessionLogPath) {
        unlink(exitSession.codexSessionLogPath).catch(() => {
          // Best-effort. Wrapper trap also removes it; either side may win.
        })
      }
