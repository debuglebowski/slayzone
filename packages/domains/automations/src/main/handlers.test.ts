 ...
 *
 * Handlers run on the async `SlayzoneDb` proxy (worker-thread DB), so they're
 * registered with `h.slayDb` and every `h.invoke(...)` returns a Promise — each
 * call is awaited. Fixture setup still uses the raw sync `h.db`.
 */
import {
  createTestHarness,
  test,
...
registerAutomationHandlers(h.ipcMain as never, h.slayDb, mockEngine)
...
await describe('db:automations:create', () => {
  test('creates with required fields', async () => {
    const a = (await h.invoke('db:automations:create', {
  ...
  test('conditions defaults to empty array', async () => {
    const a = (await h.invoke('db:automations:create', {
      project_id: projectId,
      name: 'Auto 3',
      trigger_config: triggerConfig,
  ...
  test('auto-increments sort_order per project', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    expect(all[0].sort_order).toBe(0)
    expect(all[1].sort_order).toBe(1)
    expect(all[2].sort_order).toBe(2)
  })

  test('different project starts at sort_order 0', async () => {
    const a = (await h.invoke('db:automations:create', {
  ...
  test('stores conditions when provided', async () => {
    const conditions = [
...
await describe('db:automations:getByProject', () => {
  test('returns automations for project', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    expect(all.length).toBeGreaterThan(0)
    for (const a of all) expect(a.project_id).toBe(projectId)
  })

  test('does not return other projects automations', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId2)) as Automation[]
    for (const a of all) expect(a.project_id).toBe(projectId2)
  })

  test('returns empty array for nonexistent project', async () => {
    const all = (await h.invoke('db:automations:getByProject', 'nonexistent')) as Automation[]
    expect(all).toHaveLength(0)
  })

  test('ordered by sort_order', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
...
await describe('db:automations:get', () => {
  test('returns automation by ID', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    const a = (await h.invoke('db:automations:get', all[0].id)) as Automation
    expect(a.id).toBe(all[0].id)
    expect(a.name).toBe(all[0].name)
  })

  test('returns null for nonexistent', async () => {
    const a = await h.invoke('db:automations:get', 'nonexistent')
    expect(a).toBeNull()
  })
})
...
await describe('db:automations:update', () => {
  test('updates name only', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    const orig = all[0]
    const updated = (await h.invoke('db:automations:update', {
      id: orig.id,
      name: 'Renamed'
    })) as Automation
  ...
  test('updates enabled boolean→integer', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    const a = (await h.invoke('db:automations:update', {
      id: all[0].id,
      enabled: false
    })) as Automation
    expect(a.enabled).toBe(false)
    const b = (await h.invoke('db:automations:update', {
      id: all[0].id,
      enabled: true
    })) as Automation
  ...
  test('updates trigger_config', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
  ...
  test('no fields → automation unchanged', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    const before = all[0]
    const after = (await h.invoke('db:automations:update', { id: before.id })) as Automation
    expect(after.name).toBe(before.name)
  })

  test('updates conditions', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    const newConditions = [
  ...
  test('updates sort_order', async () => {
    ...
    // Reset
    await h.invoke('db:automations:update', { id: all[0].id, sort_order: 0 })
  })
})
...
await describe('db:automations:delete', () => {
  test('deletes existing → true', async () => {
    const a = (await h.invoke('db:automations:create', {
      project_id: projectId,
      name: 'ToDelete',
      trigger_config: triggerConfig,
    ...
    expect(await h.invoke('db:automations:delete', a.id)).toBe(true)
  })

  test('nonexistent → false', async () => {
    expect(await h.invoke('db:automations:delete', 'nonexistent')).toBe(false)
  })

  test('CASCADE deletes related runs', async () => {
    const a = (await h.invoke('db:automations:create', {
      project_id: projectId,
      name: 'WithRuns',
      trigger_config: triggerConfig,
    ...
    await h.invoke('db:automations:delete', a.id)
...
await describe('db:automations:toggle', () => {
  test('toggles enabled on/off', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    const a = (await h.invoke('db:automations:toggle', all[0].id, false)) as Automation
    expect(a.enabled).toBe(false)
    const b = (await h.invoke('db:automations:toggle', all[0].id, true)) as Automation
    expect(b.enabled).toBe(true)
  })
})
...
await describe('db:automations:reorder', () => {
  test('reorders by ID array', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    const reversed = [...all].reverse().map((a) => a.id)
    await h.invoke('db:automations:reorder', reversed)
    const after = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
  ...
  test('empty array → no-op', async () => {
    const before = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    await h.invoke('db:automations:reorder', [])
    const after = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
...
await describe('db:automations:getRuns', () => {
  test('returns empty for no runs', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    const runs = (await h.invoke('db:automations:getRuns', all[0].id)) as AutomationRun[]
    // May have runs from other tests, but should not error
    expect(Array.isArray(runs)).toBe(true)
  })

  test('returns runs ordered by started_at DESC', async () => {
    const a = (await h.invoke('db:automations:create', {
      project_id: projectId,
      name: 'RunTest',
      trigger_config: triggerConfig,
    ...
    h.db
      ...
      .run('r2', a.id)
    const runs = (await h.invoke('db:automations:getRuns', a.id)) as AutomationRun[]
  ...
  test('respects custom limit', async () => {
    ...
    const runs = (await h.invoke('db:automations:getRuns', a.id, 2)) as AutomationRun[]
    expect(runs).toHaveLength(2)
  })
...
await describe('db:automations:clearRuns', () => {
  test('deletes all runs for automation', async () => {
    const a = (await h.invoke('db:automations:create', {
      project_id: projectId,
      name: 'ClearTest',
      trigger_config: triggerConfig,
    ...
    h.db
      ...
      .run('cr-2', a.id)
    await h.invoke('db:automations:clearRuns', a.id)
    const runs = (await h.invoke('db:automations:getRuns', a.id)) as AutomationRun[]
    expect(runs).toHaveLength(0)
  })

  test('does not affect other automations runs', async () => {
    const a1 = (await h.invoke('db:automations:create', {
      project_id: projectId,
      name: 'Clear1',
      trigger_config: triggerConfig,
      actions
    })) as Automation
    const a2 = (await h.invoke('db:automations:create', {
      project_id: projectId,
      name: 'Clear2',
      trigger_config: triggerConfig,
      actions
    ...
    h.db
      ...
      .run('iso-2', a2.id)
    await h.invoke('db:automations:clearRuns', a1.id)
    const runs1 = (await h.invoke('db:automations:getRuns', a1.id)) as AutomationRun[]
    const runs2 = (await h.invoke('db:automations:getRuns', a2.id)) as AutomationRun[]
...
await describe('db:automations:reorder edge cases', () => {
  test('duplicate IDs → last occurrence wins sort_order', async () => {
    const all = (await h.invoke('db:automations:getByProject', projectId)) as Automation[]
    if (all.length < 2) return
    const dup = [all[0].id, all[0].id, all[1].id]
    await h.invoke('db:automations:reorder', dup)
    const a0 = (await h.invoke('db:automations:get', all[0].id)) as Automation
    const a1 = (await h.invoke('db:automations:get', all[1].id)) as Automation
...
await describe('db:automations:getRuns edge cases', () => {
  test('limit=0 returns no runs (SQL LIMIT 0)', async () => {
    const a = (await h.invoke('db:automations:create', {
      project_id: projectId,
      name: 'Limit0Test',
      trigger_config: triggerConfig,
    ...
    h.db
      ...
      .run('lz-1', a.id)
    const runs = (await h.invoke('db:automations:getRuns', a.id, 0)) as AutomationRun[]
    expect(runs).toHaveLength(0)
  })
})
...
await describe('project deletion cascade', () => {
  test('deleting project cascades to its automations', async () => {
    ...
    const result = await h.invoke('db:automations:get', a.id)
