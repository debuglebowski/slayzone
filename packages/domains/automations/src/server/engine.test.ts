...
import { tagsEvents } from '@slayzone/tags/server'
...
await describe('task_tag_changed trigger', () => {
  test('fires automation on tag change event', async () => {
    ...
    tagsEvents.emit('tags:set-for-task', { taskId, tagIds: ['some-tag'] })
    await new Promise(r => setTimeout(r, 500))
    const runs = getRuns(id)
    expect(runs.length).toBeGreaterThan(0)
    expect(runs[0].status).toBe('success')
  ...
  test('nonexistent task → skipped gracefully', () => {
    ...
    tagsEvents.emit('tags:set-for-task', { taskId: 'nonexistent', tagIds: ['tag'] })
  })
})
