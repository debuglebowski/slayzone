...
beforeEach(() => {
  baseDir = mkdtempSync(join(tmpdir(), 'fleet-creds-'))
})
...
describe('credential file naming', () => {
  it('hubHostFromUrl folds host and port', () => {
    expect(hubHostFromUrl('wss://hub.example:8443/fleet')).toBe('hub.example_8443')
    expect(hubHostFromUrl('wss://hub.example/fleet')).toBe('hub.example')
