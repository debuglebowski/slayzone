 ...
 *
 * @module fleet/client/hub-dialer
 */
...
export class HubDialer {
  ...
  constructor(opts: HubDialerOptions) {
    ...
    this.events = new TypedEventEmitter<HubDialerEvents>((event, err) => {
      this.log('fleet dialer listener threw', { event: String(event), error: String(err) })
    })
  ...
  request<T = unknown>(method: string, params?: unknown, timeoutMs?: number): Promise<T> {
    ...
    if (!conn?.authenticated || conn.rpc.isDisposed) {
      return Promise.reject(new Error('fleet dialer is not connected to the hub'))
    }
   ...
   * so no fleet frame (hello/enroll credentials) is ever sent to an unpinned
  ...
  private verifyPinnedCert(ws: WebSocket, tlsSocket: TLSSocket): boolean {
    ...
    this.log('fleet dialer rejected hub certificate', { error: error.message })
  ...
  private connect(): void {
    ...
    conn.rpc = new DuplexRpc({
      ...
      onParseError: (line, err) => {
        this.log('fleet dialer received malformed frame', { error: String(err), line })
      }
    })
    this.current = conn
    ...
    ws.on('error', (err) => {
      ...
      this.log('fleet dialer socket error', { error: String(err) })
    ...
    try {
      ...
      if (stored) {
        try {
          ...
          if (!(err instanceof RpcError) || err.code !== FleetErrorCodes.unauthorized) throw err
          ...
          this.log('fleet dialer hello rejected, re-enrolling', { code: err.code })
        }
      }
      if (!this.opts.joinToken) {
      ...
      try {
        ...
        if (
          ...
          (err.code === FleetErrorCodes.unauthorized || err.code === FleetErrorCodes.protocolMismatch)
        ) {
      ...
      try {
        ...
        this.log('fleet dialer credential save failed', { error: error.message })
        this.events.emit('error', { error, fatal: false })
      }
      ...
      this.log('fleet dialer authentication interrupted', { error: String(err) })
      conn.ws.terminate()
    }
  }
  ...
  private onAuthenticated(conn: Connection, runnerId: string, mode: 'enroll' | 'hello'): void {
    ...
    this.log('fleet dialer connected', { runnerId, mode })
  }
    ...
    conn.heartbeatTimer = setInterval(() => {
      ...
      conn.rpc
        ...
        .catch((err: unknown) => {
          ...
          this.log('fleet dialer heartbeat failed, dropping connection', { error: String(err) })
          conn.ws.terminate()
        })
    }, intervalMs)
  ...
  private handleHubRequest(conn: Connection, method: string, params: unknown, id: string | number): void {
    ...
    if (!handler) {
      conn.rpc.respondError(id, FleetErrorCodes.unimplemented, `unimplemented: ${method}`)
      return
    }
    void (async () => {
      try {
  ...
  private fatal(error: Error, conn: Connection): void {
    ...
    this.log('fleet dialer fatal error', { error: error.message })
    conn.ws.terminate()
  }
}
