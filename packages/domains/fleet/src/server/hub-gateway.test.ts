...
import { FleetErrorCodes } from '../shared/frames'
...
describe('enrollment', () => {
  ...
  it('bad join token: enrollment is refused with unauthorized and the dialer gives up (fatal)', async () => {
    ...
    expect(error.message).toContain(String(FleetErrorCodes.unauthorized))
  ...
  it('protocol version mismatch is refused with protocolMismatch', async () => {
    ...
    expect((frame.error as { code: number }).code).toBe(FleetErrorCodes.protocolMismatch)
  })
})
...
describe('hub → runner requests', () => {
  ...
  it('requests to unknown runners reject with unknownRunner', async () => {
    ...
    expect(err).toBeInstanceOf(RpcError)
    expect((err as RpcError).code).toBe(FleetErrorCodes.unknownRunner)
  })
...
describe('malformed frames', () => {
  ...
  it('pre-auth requests other than enroll/hello are refused with unauthorized', async () => {
    ...
    expect(((await reply).error as { code: number }).code).toBe(FleetErrorCodes.unauthorized)
  })
