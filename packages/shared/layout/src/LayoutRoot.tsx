...
import { getLayoutStore, useLayoutTree } from './store'
      ...
      {tree.root ? (
        <>
          ...
          {resolved.dividers.map((d) => {
            ...
            const totalContentPx = Math.max(0, total - DIVIDER_PX * Math.max(0, split.children.length - 1))
    ...
    >
      ...
      >
        {pane.tiles.length > 1 ? (
          ...
          <span style={{ padding: '8px', fontSize: 12, fontWeight: 600, color: '#cfcfd4' }}>
            {active?.title}
          </span>
        )}
      </header>
