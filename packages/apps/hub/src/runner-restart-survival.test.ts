/**
 * Runner restart-survival: local runner stays ONE row across reboots (Wave3.5-D5).
 *
 ...
 * of orphaning a fresh one. The bug: the `/runners` listener bound an OS-assigned
 ...
 *   - PART 1 port pinning: `resolveDesiredRunnerPort` + `claimRunnerServerPort`
 *     (persist to `settings.runner_transport_port`) over a real migrated SlayzoneDb.
 ...
 *
 * A `simulateBoot` models exactly one sidecar boot: resolve the desired runner
 ...
 * `wss://host:<port>/runners`, then run the runner's authenticate logic (mirrors
 ...
 *     packages/apps/hub/src/runners-restart-survival.test.ts
 */
...
import { claimRunnerServerPort, resolveDesiredRunnerPort } from './port-claim.js'
...
async function simulateBoot(deps: BootDeps): Promise<BootResult> {
  // PART 1: resolve the desired runner port (env > persisted > 0), "bind" it, and
  // persist the actually-bound port for the next boot.
  const desired = await resolveDesiredRunnerPort(deps.db, deps.runnerPortEnv)
  ...
  await claimRunnerServerPort(deps.db, HOST, boundPort, () => {}, { force: fellBackFromPinned })

  const hubUrl = `wss://${HOST}:${boundPort}/runners`
  ...
  try {
    await test('stable pinned port ⇒ boot 2 HELLOs into the same row (count stays 1)', async () => {
      ...
      try {
        ...
        // Boot 2: PART 1 makes resolveDesiredRunnerPort return the persisted port,
    ...
    await test('pinned port TAKEN ⇒ fallback port is force-persisted; next boot HELLOs (stays 1)', async () => {
      ...
      try {
        ...
        const persisted = await resolveDesiredRunnerPort(db, undefined)
    ...
    await test('regression guard: WITHOUT port pinning the credential key changes each boot', async () => {
      ...
      try {
        ...
        const url1 = `wss://${HOST}:${fakePort}/runners`
        ...
        fakePort += 1
        const url2 = `wss://${HOST}:${fakePort}/runners`
    ...
    await test('operator SLAYZONE_HUB_RUNNER_TRANSPORT_PORT override pins directly (also stays 1 across boots)', async () => {
      ...
      const bindPort = makePortAllocator(51000)
      try {
        const boot1 = await simulateBoot({ db, auth, credsBaseDir, bindPort, runnerPortEnv: '51234', localRunnerName: LOCAL_NAME })
        assertEq(boot1.boundPort, 51234, 'env override port bound')
        const boot2 = await simulateBoot({ db, auth, credsBaseDir, bindPort, runnerPortEnv: '51234', localRunnerName: LOCAL_NAME })
