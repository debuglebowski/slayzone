/**
 * Runner auth adapter factory.
 *
 * Pure glue between three dark hub/runner-split domains and the runner hub
 ...
 * unit calls `createRunnerAuthAdapters(...)` and hands the result to
 * `createHubRunnerGateway`.
 *
 * The return type is structurally pinned to `HubRunnerGatewayOptions` so the
 * callbacks can never silently drift from what the gateway expects.
 *
 * @module server/runner-auth
 */

import { randomUUID } from 'node:crypto'
import type { HubRunnerGatewayOptions } from '@slayzone/runner-transport/server'
...
/**
 * The two auth callbacks the runner hub gateway injects, sliced straight off the
 ...
 */
export type RunnerAuthAdapters = Pick<HubRunnerGatewayOptions, 'verifyEnrollment' | 'verifyApiKey'>

export interface RunnerAuthAdapterDeps {
 ...
 */
export function createRunnerAuthAdapters(deps: RunnerAuthAdapterDeps): RunnerAuthAdapters {
  const { db, auth } = deps
  ...
  const verifyEnrollment: RunnerAuthAdapters['verifyEnrollment'] = async (params) => {
    const at = now()
    sweepExpired(at)
    ...
    if (!verified.ok) {
      throw new RpcError(
        RunnerTransportErrorCodes.unauthorized,
  ...
  const verifyApiKey: RunnerAuthAdapters['verifyApiKey'] = async (apiKey) => {
