...
test.describe('Provider config edge cases', () => {
  ...
  test('set conversationId on task with empty provider_config', async ({ mainWindow }) => {
    ...
    const fresh = await mainWindow.evaluate(
      (pid) => getTrpcVanillaClient().task.create.mutate({ projectId: pid, title: 'Fresh config task' }),
      projectId
    )
    ...
    const task = await mainWindow.evaluate((id) => getTrpcVanillaClient().task.get.query({ id: id }), fresh!.id)
  ...
  test('subtask does not inherit parent conversationId', async ({ mainWindow }) => {
    ...
    const subtask = await mainWindow.evaluate(
      ({ pid, parentId }) => getTrpcVanillaClient().task.create.mutate({
        projectId: pid,
        title: 'Child task',
        parentId,
      }),
    ...
    const child = await mainWindow.evaluate((id) => getTrpcVanillaClient().task.get.query({ id: id }), subtask!.id)
