...
test.describe('Terminal session isolation', () => {
  ...
  test.beforeAll(async ({ mainWindow }) => {
    ...
    await mainWindow.evaluate((ids) => {
      return Promise.all([
        getTrpcVanillaClient().task.update.mutate({ id: ids.a, terminalMode: 'terminal' }),
        getTrpcVanillaClient().task.update.mutate({ id: ids.b, terminalMode: 'terminal' }),
      ])
    }, { a: taskAId, b: taskBId })
  ...
  test('keeps PTY output isolated across tabs and tasks', async ({ mainWindow }) => {
    ...
    await expect
      .poll(async () => {
        const tabs = await mainWindow.evaluate((id) => getTrpcVanillaClient().taskTerminals.list.query({ taskId: id }), taskAId)
        const nonMain = tabs.find((tab: { id: string; isMain: boolean }) => !tab.isMain)
        return nonMain?.id ?? null
      })
      .not.toBeNull()

    const taskATabs = await mainWindow.evaluate((id) => getTrpcVanillaClient().taskTerminals.list.query({ taskId: id }), taskAId)
