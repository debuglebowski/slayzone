...
# (produced by `pnpm build:chromium`). The bind address is pinned to
# 127.0.0.1:8766 (dev) so the renderer's baked-in default WS URL resolves without
# extra plumbing. SLAYZONE_SUPERVISED stays unset → the server does not
...
if [[ "${SLAYZONE_NO_SIDECAR:-0}" != "1" ]]; then
  # Force the bind address the renderer's baked-in default expects (window-api-shim
  # server-url.ts → ws://127.0.0.1:8766/trpc). We deliberately do NOT honor an
  # inherited SLAYZONE_HUB_ADDRESS: the renderer hardcodes the URL and has no
  # override channel yet, so the sidecar MUST match it. A leaked value — e.g. from
  # inside a running supervised SlayZone process, which sets its own sidecar's
  # address — would otherwise make this bind the wrong port (and collide:
  # EADDRINUSE), leaving the renderer with nothing to connect to.
  # Relax once the --slayzone-server-url flag is wired through the C++ shell.
  HUB_HOST=127.0.0.1
  HUB_PORT=8766
  SLAYZONE_HUB_ADDRESS="$HUB_HOST:$HUB_PORT"
  export SLAYZONE_HUB_ADDRESS
  ...
  echo "[run] starting sidecar (tRPC ws://$HUB_HOST:$HUB_PORT/trpc, log=$SIDECAR_LOG)" >&2
  ...
  # Identity check: is the process listening on $HUB_PORT one WE spawned?
  ...
  port_owner_is_ours() {
    ...
    local owner cur
    owner="$(lsof -nP -iTCP:"$HUB_PORT" -sTCP:LISTEN -t 2>/dev/null | head -1)"
    [[ -z "$owner" ]] && return 1
    cur="$owner"
  ...
  for _ in $(seq 1 80); do
    if curl -sf "http://$HUB_HOST:$HUB_PORT/health" >/dev/null 2>&1 && port_owner_is_ours; then
      ready=1
      break
    fi
  ...
  if [[ "$ready" != "1" ]]; then
    foreign="$(lsof -nP -iTCP:"$HUB_PORT" -sTCP:LISTEN -t 2>/dev/null | head -1)"
    if [[ -n "$foreign" ]] && ! port_owner_is_ours; then
      echo "[run] FATAL: port $HUB_PORT is held by a FOREIGN process (pid=$foreign), not the sidecar we spawned (pid=$SIDECAR_PID)." >&2
      ps -o pid,ppid,command -p "$foreign" >&2 2>/dev/null || true
      echo "[run] The renderer hard-pins ws://localhost:$HUB_PORT, so it would connect to that process's DB — likely an orphaned sidecar from a prior run or another SlayZone instance." >&2
